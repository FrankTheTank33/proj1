...
char decrypt_rotate2 (char c, int k2);

char decrypt_rotate3 (char c, int k3);

char decrypt_rotate4 (char c, int k4);

char decrypt_rotate5 (char c, int k5);
...
case 5: { //Rotational Decryption Without Given Key
   FILE *input;
    ...
    int k2 = 0;
    k2 = characters[maxIndex] - 84; //T is the second most common letter. It may be the most common character in the cipher.
    printf("A possible key is %d.\n", k2);
    
    int k3 = 0;
    k3 = characters[maxIndex] - 65; //A is another common letter. It may be the most common character in the cipher.
    printf("A possible key is %d.\n", k3);
    
    int k4 = 0;
    k4 = characters[maxIndex] - 73; //I is another common letter. It may be the most common character in the cipher.
    printf("A possible key is %d.\n", k4);
    
    int k5 = 0;
    k5 = characters [maxIndex] - 78; //N is another common letter. It may be the most common character in the cipher.
    printf("A possible key is %d.\n", k5);
    ...
    while (feof(input) == 0) {  // Loop which reads each byte from the Input file, puts it through decryption function and prints decrypted letter in Output2 file. 
        ...
        fscanf(input, "%c", &c); //Reads byte from the Input file.
            ...
            d = decrypt_rotate2(c, k2); //Inserts byte into decryption function.
    ...
    while (feof(input) == 0) {  // Loop which reads each byte from the Input file, puts it through decryption function and prints decrypted letter in Output3 file. 
        ...
        fscanf(input, "%c", &c); //Reads byte from the Input file.
            ...
            d = decrypt_rotate3(c, k3); //Inserts byte into decryption function using thirt possible key.
    ...
    while (feof(input) == 0) {  // Loop which reads each byte from the Input file, puts it through decryption function and prints decrypted letter in Output4 file. 
        ...
        fscanf(input, "%c", &c); //Reads byte from the Input file.
            ...
            d = decrypt_rotate4(c, k4); //Inserts byte into decryption function using fourth possible key.
    ...
    while (feof(input) == 0) {  // Loop which reads each byte from the Input file, puts it through encryption function and prints encrypted letter in Output5 file. 
        ...
        fscanf(input, "%c", &c); //Reads byte from the Input file.
            ...
            d = decrypt_rotate5(c, k5); //Inserts byte into decryption function using fifth possible key.
...
char decrypt_rotate2 (char c, int k2) { //Uses 2nd possible key for rotational decryption.
    if ((c<65) || (c>90)) 
    return c;
    //For all non-letter characters, the function does not change them.
    if (c>=65 && c<=90) {  // For all uppercase letters.
        if (c-65-k2<0)
            return ((((c-65)-k2)+26)%26)+65; 
            //In case the ASCII value is -ve, 26 is added as "%" does not function with -ves.
        if (c-65-k2>=0)
            return (((c-65)-k2)%26)+65; 
    }
    /*ASCII value reduced by 65 so that A=0...Z=25. 
    This value is changed by the key. 
    The remainder of this value and 26 is found. 
...
char decrypt_rotate3 (char c, int k3) { //Uses 3rd possible key for rotational decryption.
    if ((c<65) || (c>90)) 
    return c;
    //For all non-letter characters, the function does not change them.
    if (c>=65 && c<=90) {  // For all uppercase letters.
        if (c-65-k3<0)
            return ((((c-65)-k3)+26)%26)+65; 
            //In case the ASCII value is -ve, 26 is added as "%" does not function with -ves.
        if (c-65-k3>=0)
            return (((c-65)-k3)%26)+65; 
    }
    /*ASCII value reduced by 65 so that A=0...Z=25. 
    This value is changed by the key. 
    The remainder of this value and 26 is found. 
...
char decrypt_rotate4 (char c, int k4) {  //Uses 4th possible key for rotational decryption.
    if ((c<65) || (c>90)) 
    return c;
    //For all non-letter characters, the function does not change them.
    if (c>=65 && c<=90) {  // For all uppercase letters.
        if (c-65-k4<0)
            return ((((c-65)-k4)+26)%26)+65; 
            //In case the ASCII value is -ve, 26 is added as "%" does not function with -ves.
        if (c-65-k4>=0)
            return (((c-65)-k4)%26)+65; 
    }
    /*ASCII value reduced by 65 so that A=0...Z=25. 
    This value is changed by the key. 
    The remainder of this value and 26 is found. 
...
char decrypt_rotate5 (char c, int k5) {  //Uses 5th possible key for rotational decryption.
    if ((c<65) || (c>90)) 
    return c;
    //For all non-letter characters, the function does not change them.
    if (c>=65 && c<=90) {  // For all uppercase letters.
        if (c-65-k5<0)
            return ((((c-65)-k5)+26)%26)+65; 
            //In case the ASCII value is -ve, 26 is added as "%" does not function with -ves.
        if (c-65-k5>=0)
            return (((c-65)-k5)%26)+65; 
    }
    /*ASCII value reduced by 65 so that A=0...Z=25. 
    This value is changed by the key. 
    The remainder of this value and 26 is found. 
